...
EXPLORE_PROBABILITIES = [0.2, 0.2, 1]
...
dqn = torch.load('dqn_exploratory')
...
for epoch in range(EPOCHS):
    ...
    for episode in range(EPISODES_PER_EPOCH):
        ...
        while True:
            ...
            data = np.empty(IMAGE_SIZE * IMAGE_SIZE + 1)
            data[:IMAGE_SIZE*IMAGE_SIZE] = history[0, :, :].reshape(IMAGE_SIZE * IMAGE_SIZE)
            data[IMAGE_SIZE * IMAGE_SIZE] = action
            dataset.append(data)
            # env.render()
            # time.sleep(0.03)
            if done:
    ...
    print(dataset.shape)
    print(dataset[:2])
    np.save(f'Data/Data-{epoch}', dataset)
